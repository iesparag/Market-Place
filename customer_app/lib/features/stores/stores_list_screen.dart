...
class StoresListScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(titleSpacing: 12, title: const SearchBarPill(hint: 'Search stores & products…'), actions: const [CartButton()]),
      body: async.when(
        ...
        child: Row(children: [
          Container(
            ...
            child: vendor.logo != null
                ? CachedNetworkImage(imageUrl: vendor.logo!, fit: BoxFit.cover, width: 56, height: 56)
                : Text(vendor.name.isNotEmpty ? vendor.name[0] : '?', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 22)),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(vendor.name, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
                ...
                if (vendor.ratingCount > 0)
                  Padding(
                    ...
                    child: Row(children: [
                      ...
                      Text(' ${vendor.ratingAvg.toStringAsFixed(1)} (${vendor.ratingCount})', style: const TextStyle(fontSize: 12)),
                    ]),
                  ),
              ],
            ),
