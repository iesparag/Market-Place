...
export interface NavItemMeta {
  ...
  label: string;
  route: string;
  /** Only show when the user is scoped to a store (has a storeId) — e.g. a vendor's own store. */
...
export const NAV_META: NavSectionMeta[] = [
  {
    ...
    items: [
      ...
      { perm: 'product:read', label: 'Inventory', route: '/inventory' },
  ...
  },
  {
    section: 'Vendors',
    items: [
      { perm: 'store:read', label: 'Stores', route: '/stores' },
      // Vendor-only: edit their own storefront (branding, banner carousel, KYC). Hidden for admins (no storeId).
      { perm: 'store:update', label: 'My Store', route: '/my-store', requiresStore: true },
    ],
  },
  {
    section: 'Orders',
  ...
  },
  {
    section: 'Reviews',
    items: [
      { perm: 'review:read', label: 'Reviews', route: '/reviews' },
      { perm: 'product:read', label: 'Questions', route: '/questions' },
    ],
  },
  {
    section: 'Marketing',
